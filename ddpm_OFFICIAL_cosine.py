...
images0 = torch.cat(images0, dim=0).view(-1, 3, 64, 64)
clss = torch.cat(images1, dim=0).view(-1, 1)
...
class CondDDPM(nn.Module):
    def __init__(self, schedueler, network, n_steps=1000, min_beta=10 ** -4, max_beta=0.02, device=None, image_chw=(1, 64, 64)):
      ...
      #  self.betas = torch.linspace(min_beta, max_beta, n_steps).to(device)
        ...
        self.alphas = None
        self.alpha_bars = []
        self.schedueler = schedueler
        if self.schedueler == "cosine":
          s=0.008
        ...
        #return torch.clip(betas, 0.0001, 0.9999)
          ...
          self.betas = torch.linspace(self.min_beta, self.max_beta, self.n_steps).to(device)
          self.alphas = (1 - self.betas).to(device)
          self.alpha_bars = []
          for i, a in enumerate(self.alphas):
            if i == 0:
...
ddpm = CondDDPM("cosine", ConditionalUNet(context_dim=512), device=device)
ema = EMA()
    ...
    for epoch in tqdm(range(n_epochs), desc=f"Training progress", colour="#00ff00"):
        ...
        for step, batch in enumerate(tqdm(loader, leave=False, desc=f"Epoch {epoch + 1}/{n_epochs}", colour="#005500")):
          if step != len(loader) - 1:
            ...
            eta_theta = ddpm.backward(noisy_imgs, t.reshape(n, -1), clss)
      ...
      #      show_images(generate_new_images(ddpm, device=device), f"Images generated at epoch {epoch + 1}")
       ...
       # if best_loss > epoch_loss:
        ...
        torch.save(ema_model.state_dict(), "ddpm_only_label_ema_cosine.pt")
...
model = CondDDPM("cosine", ConditionalUNet(context_dim=512), n_steps=1000, device=device)

training_loop(model, loader, n_epochs, device, display=False, store_path="ddpm_only_label_cosine.pt")
