    ...
    if clss != None:
      #  print("SHAPE OF CLASS:", "SHAPE OF TIME:", t.shape, "SHAPE OF EMBEDDINGS:", label.shape)
       ...
       # print("shape of clss:", clss.shape, "shape of embs:", label.shape)
        ...
        for zero in zero_indices:
            clss[zero] = 0 #batch.size*0.2 of batch will be set to zero

        n = len(x)
    #print("it may go wrong here wopsey;", label.shape, clss.shape)
    if label != None and clss != None:
      label += clss
      t+=label
    if label != None and clss == None:
      t+=label
    ...
    for epoch in tqdm(range(n_epochs), desc=f"Training progress", colour="#00ff00"):
      ...
      #      show_images(generate_new_images(ddpm, device=device), f"Images generated at epoch {epoch + 1}")
       ...
       # if best_loss > epoch_loss:
        ...
        torch.save(ema_model.state_dict(), "ddpm_resnet_label_ema22.pt")
