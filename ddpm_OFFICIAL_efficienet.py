...
embeddings = torch.cat(embeddings, dim=0).view(-1, 2560)
...
ddpm = CondDDPM(ConditionalUNet(context_dim=2560), device=device)
ema = EMA()
...
model = CondDDPM(ConditionalUNet(context_dim=2560), n_steps=1000, device=device)
