...
images = torch.load('efficientnet_b7_img_emo_emb.pt') # [elem, elem] elem: [bilde_batch, emo_batch, path_batch, embeddings_batch]
imgs = [] #images
classes = [] #classes
embeddings = []
for im in images:
  for i in range(len(im[0])):
    imgs.append(im[0][i])
    classes.append(im[1][i])
    embeddings.append(im[2][i])

lst = []
for i in range(len(embeddings)):
   lst.append([imgs[i], classes[i], embeddings[i]])

distinct_emo = [0,1,2,3,4,5,6]
dicit = {0: [], 1: [], 2: [], 3: [], 4: [], 5: [], 6: []}

def all_lists_have_ten_elements(d):
    return all(len(lst) == 10 for lst in d.values())
...
def generate_new_images(ddpm, labels, n_samples=10, device=None, frames_per_gif=100, gif_name="sampling.gif", c=1, h=64, w=64):
    k = 3.0
...
whole_lst = [] #liste med liste. en liste inni listen: [emotion, emotion] emotion: [elem, elem] elem= [tnsr, emo, generated]
for key in dicit:
    emo_lst = []
    for emo in dicit[key]:
        ...
        emo_lst.append([emo[0], emo[1], generated])
    whole_lst.append(emo_lst)

torch.save(whole_lst, 'efficientnet_b7_generated.pt')
