...
model_path = 'ddpm_only_label_ema.pt'
model = CondDDPM(ConditionalUNet(context_dim=512), device=device)  # Replace with your model definition
state_dict = torch.load(model_path)
model.load_state_dict(state_dict)


def generate_new_images(ddpm, emo, n_samples=100, device=None, frames_per_gif=100, gif_name="sampling.gif", c=1, h=64, w=64):
    k = 5.0
...
for emo in dicit:
  ...
  print(emo.shape, "is the shape of the only conditions that actually goes into the model")
...
torch.save(generated_lst, 'only_label_generated_test.pt') #liste med følelser. hvert følelse: img_lst, emo_lst, path_lst, generated. skjekke typen til generated, men tror det er en tensor med 100,1,64,64
print("saved, done running")
