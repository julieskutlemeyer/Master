...
class CondDDPM(nn.Module):
    ...
    def backward(self, x, t, cond=None, clss=None):
...
#under sampling: kommer til å sette label til None
  def forward(self, x, t,  label=None, clss=None):
      ...
      if label != None:
        ...
        clss = self.label_enc(clss) # får nå også shape (16,100)
        ...
        print("shape of clss:", clss.shape, "shape of embs:", label.shape)
...
model_path = 'ddpm_resnet_label_ema.pt'
model = CondDDPM(ConditionalUNet(context_dim=512), device=device)  # Replace with your model definition
state_dict = torch.load(model_path)
model.load_state_dict(state_dict)


def generate_new_images(ddpm, labels, emo, n_samples=100, device=None, frames_per_gif=100, gif_name="sampling.gif", c=1, h=64, w=64):
    k = 5.0
    ...
    with torch.no_grad():
        ...
        emb_label = labels.to(device)
        emo_label = emo.to(device)
        ...
        for idx, t in enumerate(list(range(ddpm.n_steps))[::-1]): #iterere fra denoising steps to 0, gjøre classifier-free sampling per gang
            ...
            eta_theta_cond = ddpm.backward(x, time_tensor, emb_label, emo_label) * (1 + k)
            eta_theta_uncond = ddpm.backward(x, time_tensor, torch.zeros_like(emb_label), torch.zeros_like(emo_label)) * k #torch.zeros(10, 1).to(device)
...
torch.save(generated_lst, 'resnet_label_generated_test.pt') #liste med følelser. hvert følelse: img_lst, emo_lst, path_lst, generated. skjekke typen til generated, men tror det er en tensor med 100,1,64,64
