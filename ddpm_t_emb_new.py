...
images  = torch.load('images_64_fer2.pt') #from  image_embeddings
images0 = []
for im in images:
  images0.append(im[0])
...
print(embeddings.shape, images0.shape)
...
class CustomDataset2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_tensor = F.interpolate(img_tensor.unsqueeze(0), size=(64, 64), mode='bilinear', align_corners=False)
   ...
   #     class_id = torch.tensor([class_id])
        class_id = self.embs[idx]
        return img_tensor, class_id

data2 = CustomDataset2()
loader = DataLoader(data2, 60, shuffle=True)


for batch in loader:
    print("shape of images:", batch[0].shape, "shape of img embeds: ", batch[1].shape)
    break
...
class CrossAttention(nn.Module):
  def __init__(self, embed_dim, hidden_dim, context_dim=None, num_heads=1,): #embed_dim: dim til bilde(1 eller 3 osv) , hiddem_dim:dim til bilde, context_dim:
    ...
    self.num_heads = self.embed_dim // 5 if embed_dim != 1 else 1
    ...
    for epoch in tqdm(range(n_epochs), desc=f"Training progress", colour="#00ff00"):
      ...
      #      show_images(generate_new_images(ddpm, device=device), f"Images generated at epoch {epoch + 1}")
        ...
        if best_loss > epoch_loss:
            ...
            torch.save(ema_model.state_dict(), "ddpm_with_ema_t+l_no1sqrt2.pt")
...
training_loop(model, loader, n_epochs, device, display=False, store_path="ddpm_fer2013_im2vec_siluresnet_t+l.pt")
